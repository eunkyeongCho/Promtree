...
class Neo4jKnowledgeGraph:
    ...
    async def _async_extract_nodes_or_relationships(self, semaphore: asyncio.Semaphore, text_to_analyze: str, need_relationships: bool) -> list[dict[str, Any]]:
        ...
        # print(f"🔍 [async_extract_nodes_or_relationships] 그래프 추출하기 위해 보낸 Prompt: \n{prompt}")
            ...
            def sync_request():
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        # {
                        #     "role": "system",
                        #     "content": """당신은 RAG의 Knowledge Graph를 구축하기 위해, 주어진 문자열에서 주요개념과 그 관계를 추출하는 전문가입니다.
                        #     아래 JSON 형식을 따라 두개의 주요개념과 그 둘 사이의 관계를 최대한 많이 추출하세요.
                ...
                # print(f"🔍 [async_extract_nodes_or_relationships] 그래프 추출결과(배열): \n{json.dumps(json.loads(response.choices[0].message.content), ensure_ascii=False, indent=2)}")

                return json.loads(response.choices[0].message.content) # 답변 그 자체
    ...
    async def async_ingest_chunks(self, chunks: list[dict]) -> bool:
        ...
        semaphore = asyncio.Semaphore(int(os.getenv("UPSTAGE_MAX_ASYNC_REQUESTS", 30)))

        tasks = []
        chunk_file_info_map = []  # 각 task에 대응하는 chunk의 file_info를 저장
        for chunk in chunks:
            ...
            chunk_file_info_map.append(chunk.get("file_info", {}))
            tasks.append(self._async_extract_nodes_or_relationships(semaphore, text_to_analyze, True))

        print(f"🚀 [async_ingest_chunks] Sending {len(tasks)} LLM requests concurrently... (max concurrent: {os.getenv('UPSTAGE_MAX_ASYNC_REQUESTS')})")
        ...
        for i, result in enumerate(all_chunks_nodes_and_relationships):
            ...
            # 해당 chunk의 file_info 가져오기
            chunk_file_info = chunk_file_info_map[i] if i < len(chunk_file_info_map) else {}
                ...
                for nodes_and_relationship in chunk_nodes_and_relationships:  # 그 list는 dict의 배열이기 때문에, 또 반복문 돌면 dict가 하나씩 나옴!
                    ...
                    try:
                        self.neo4j_driver.execute_query(
                            ...
                            file_info=json.dumps(chunk_file_info, ensure_ascii=False, indent=2),
    ...
    async def async_search_graph(self, query: str) -> list[dict[str, Any]]:
        ...
        print(f"🔍 [async_search_graph] Nodes: \n{json.dumps(nodes, ensure_ascii=False, indent=2)}")
    ...
    async def generate_answer(self, query: str) -> str:
            ...
            print(f"🔍 [generate_answer] Prompt: \n{prompt}")
            ...
            data = response.json()
            print(f"🔍 [generate_answer] LLM response: {data.get('response')}")
...
async def main():
    ...
    markdown_sample_data_folder_path = (Path(__file__).resolve().parent / "markdown_sample_data")  # markdown 샘플 데이터 경로

    print("📂 Searching md files in:", str(markdown_sample_data_folder_path))
    ...
    for markdown_file_path in markdown_sample_data_folder_path.rglob("*.md"): # md 파일만 순회돌기
        with open(markdown_file_path, "r", encoding="utf-8") as f:  # 파일로부터 md 문자열을 읽어옵니다.
            ...
            await knowledge_graph.generate_answer("톨루엔의 생식독성은?")
