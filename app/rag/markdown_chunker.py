...
from app.rag.html2row import parse_table

load_dotenv()

class MarkdownChunker:
    """
    Parser 쪽에서 생성된 markdown 파일을 chunking하고, 완성된 청크들을 Chunk DB에 저장합니다.
    """
    ...
    def save_chunks_to_db(self, chunks: list[dict]) -> bool:
        ...
        try:
            self.chunk_collection.insert_many(chunks)
    ...
    def chunk_markdown_file(self, md: str, file_uuid: str, file_name: str, collections: list[str]) -> bool:
        ...
        try:
            ...
            chunks = self.attach_file_info(raw_chunks, pages_info, file_info) # raw chunks에 file_info 추가
