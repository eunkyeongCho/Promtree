...
from retriever.vector_store.faiss.faiss_vector_store import FaissVectorStore
    ...
    def test_faiss(self):
        for markdown_file_path in self.markdown_sample_data_folder_path.rglob("*.md"): # md 파일만 순회돌기
            with open(markdown_file_path, "r", encoding="utf-8") as f:  # 파일로부터 md 문자열을 읽어옵니다.
                ...
                markdown_chunker.chunk_markdown_text_and_save_to_mongodb(md, "5bc0c676-018f-46de-bb0d-0103ff9c388c", "5bc0c676-018f-46de-bb0d-0103ff9c388c_3M-1509-DC-Polyethylene-Tape-TIS-Jun13", ["msds"]) # 임의로 하드코딩 했으므로 모든 샘플 파일의 file_info의 키 중 file_uuid, file_name, collections 값이 동일하게 청크가 만들어집니다.

                # Faiss 벡터 저장
                self.faiss.embedding_chunks(self.model, "5bc0c676-018f-46de-bb0d-0103ff9c388c", "msds")

                # Faiss 벡터 검색
                query = "아세톤의 권고 용도는?"
                query_vector = self.model.encode(query, normalize_embeddings=True, show_progress_bar=True)
                vector_results = self.faiss.search_similar_chunks("msds", query_vector, 5)
...
if __name__ == "__main__":
    faiss_test = FaissTest()
    faiss_test.test_faiss()