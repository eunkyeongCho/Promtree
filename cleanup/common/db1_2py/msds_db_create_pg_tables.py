...
def upsert_product(conn, product_name: str, company_name: str | None = None):
    sql = """
        INSERT INTO products (product_name, company_name)
        VALUES (%s, %s)
    ...
    with conn.cursor() as cur:
        cur.execute(sql, (product_name, company_name))
        row = cur.fetchone()
        if not row:
...
def save_current_parse_to_postgres(
    ...
    print(product_name, company_name)
